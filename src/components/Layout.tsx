      ...
      <aside className="hidden lg:flex flex-col w-64 bg-gradient-to-b from-slate-900 to-slate-950 border-r border-slate-800/50">
        ...
        <div className="p-6 border-b border-slate-800/50">
          <div className="flex items-center gap-3">
            <div className="relative w-11 h-11">
              <motion.div
                className="w-11 h-11 rounded-xl bg-gradient-to-br from-emerald-500/30 to-cyan-500/20 flex items-center justify-center border border-emerald-500/20"
              ...
              <img
                src="/icon-192.png"
                alt="SPECTRE icon"
                className="absolute -top-3 -right-3 w-14 h-14 rounded-xl object-cover shadow-[0_10px_30px_rgba(2,6,23,0.6)]"
              />
            </div>
            <div>
        ...
        <div className="p-4 border-t border-slate-800/50">
          <div className="p-3 rounded-xl bg-slate-800/30 border border-slate-700/30">
            ...
            <p className="text-[10px] text-slate-600 font-mono">v1.0.0 // SPECTRE PROTOCOL</p>
          </div>
        </div>
      </aside>

      {/* Main Content */}
      <main className="flex-1 flex flex-col min-h-screen min-h-dvh">
        {/* Mobile Header - Minimal Swiss Design */}
        <motion.header 
          initial={{ y: 0 }}
          animate={{ y: scrollDirection === 'down' ? -56 : 0 }}
          transition={{ duration: 0.25, ease: [0.4, 0, 0.2, 1] }}
          className="lg:hidden sticky top-0 z-30 bg-slate-950/95 backdrop-blur-xl border-b border-slate-800/30 safe-area-pt"
        >
          <div className="flex items-center justify-between px-4 h-14">
            <div className="flex items-center gap-2.5">
              <div className="relative w-8 h-8">
                <div className="w-8 h-8 rounded-xl bg-emerald-500/15" aria-hidden="true" />
                <img src="/icon-192.png" alt="SPECTRE icon" className="absolute -top-1 -left-1 w-10 h-10 rounded-sm object-cover shadow-sm" />
              </div>
              <h1 className="font-semibold text-white text-base">{pageTitles[currentPage]}</h1>
            </div>
            <button
        ...
        <div className="flex-1 overflow-auto scrollbar-hide scroll-smooth webkit-overflow-scrolling-touch">
          <div className="px-4 pt-4 pb-24 lg:pb-8 lg:px-6 lg:pt-6 w-full">
            <motion.div
              key={currentPage}
        ...
        <nav 
          className="lg:hidden fixed bottom-0 left-0 right-0 z-30 bg-slate-950/80 backdrop-blur-2xl border-t border-slate-800/30 safe-area-pb" 
        ...
        >
          <ul className="flex items-center h-[var(--spacing-xl)] px-2 pb-1 pt-1 justify-around">
            {navItems.slice(0, 4).map((item) => {
              ...
              return (
                <li key={item.id} className="flex-1 max-w-[20%]">
                  <motion.button
                    onClick={() => {
                      onNavigate(item.id)
                      vibrate()
            ...
            <li className="flex-1 max-w-[20%]">
              ...
              >
                <div className={`relative p-1.5 rounded-xl transition-all duration-300 ${
                  mobileMenuOpen ? 'bg-emerald-500/10' : 'bg-transparent'
                }`}>
        ...
        {mobileMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className="lg:hidden fixed left-0 right-0 bottom-0 bg-slate-900 z-50 rounded-t-3xl overflow-hidden max-h-[85vh]"
            >
              {/* Handle bar */}
